 ...
 *
 * A bot editing fr.wiktionary should leave French summaries. Message bundles are keyed by
 * language and support `$1` placeholders and `{{PLURAL:}}`, which languages need where a count
 * changes the word after it.
 *
 * ```
 ...
 * ```
 *
 * A key missing in the requested language falls back through [fallbacks] and then to English. A
 * key missing everywhere returns the key itself rather than throwing: a bot that has done the
 * work should not lose the edit because a translation is missing.
 */
public class Messages(
     ...
     *
     * `{{PLURAL:$1|singular|plural}}` is expanded using the rule for the language, not the rule
     * for English, which is why this is not string interpolation.
     */
     ...
     *
     * Ending in English, which every bundle in practice has, because an English summary is
     * better than a raw message key in the page history.
     */
     ...
     *
     * Written by hand rather than with a regex over the whole thing, because the forms can
     * contain the braces of another template and a regex would stop at the first `}}`.
     */
    private fun expandPlural(template: String, language: String, arguments: List<Any?>): String {
        ...
        while (true) {
            ...
            val chosen = if (count == null || forms.isEmpty()) {
     ...
     *
     * A form can hold a template or a link, and both use `|` for their own parameters. Splitting
     * on every `|` cuts `{{many|$1}}` in half and yields a form that is not one.
     */
     ...
     *
     * Counted rather than searched, so a plural form containing another template still ends in
     * the right place.
     */
     ...
     *
     * The rules are the ones that differ in a way a bot summary notices. A language this does not
     * know uses the English rule, which is what MediaWiki does with an unconfigured language and
     * what a two-form bundle expects anyway.
     */
    private fun pluralForm(count: Long, forms: List<String>, language: String): String {
        val index = when (language) {
            ...
            "fr", "hy", "ln", "pt-br" -> if (count <= 1) 0 else 1

            // Russian, Ukrainian and their neighbours: one, few, many.
            "ru", "uk", "be", "sr", "hr", "bs" -> slavicIndex(count)
     ...
     *
     * The indices are named because "3" meaning "the few form" is exactly the kind of number
     * that gets edited into the wrong branch a year later.
     */
    private fun arabicIndex(count: Long): Int = when {
        count == 0L -> ZERO_FORM
        count == 1L -> ONE_FORM
        count == 2L -> TWO_FORM
    ...
    public companion object {
         ...
         *
         * Deliberately smaller than MediaWiki's full fallback graph, which exists for interface
         * messages seen by readers. An edit summary needs only the cases where speakers of one
         * language routinely read another.
         */
        public val FALLBACKS: Map<String, List<String>> = mapOf(
            "nb" to listOf("no"),
            "nn" to listOf("nb", "no"),
            "no" to listOf("nb"),
         ...
         *
         * The format is MediaWiki's own: a flat object of key to message, with `@metadata`
         * ignored, so a bundle can be taken from translatewiki unchanged.
         */
        public fun fromJson(bundles: Map<String, String>): Messages = Messages(
