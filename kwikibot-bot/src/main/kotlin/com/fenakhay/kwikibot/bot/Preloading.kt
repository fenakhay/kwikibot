 ...
 *
 * Batched because the arithmetic demands it: fetching fifty pages one at a time is fifty round
 * trips, and the API returns all fifty in one.
 */

/**
 * Fetches the pages, in batches, as the stream is collected.
 *
 * The reason to batch is arithmetic: fetching fifty pages one at a time is fifty round trips,
 * and the API will hand over all fifty in one. Pages that do not exist are dropped, since there
 * is nothing to give a caller for them.
 *
 * The batch is filled from the stream before any request goes out, so a source that pages
 * through a category still only holds [batch] pages at a time.
 */
 ...
 *
 * Not the order the API returned it in: a bot that logs its work should log it in the order it
 * was given, and pages that do not exist simply do not appear.
 */
...
public fun Flow<PageContent>.textMatching(pattern: Regex): Flow<PageContent> =
    filter { pattern.containsMatchIn(it.text) }

/** Only pages whose text does not match [pattern]. */
public fun Flow<PageContent>.textNotMatching(pattern: Regex): Flow<PageContent> =
    filter { !pattern.containsMatchIn(it.text) }

/** Only pages that are redirects, or only pages that are not. */
public fun Flow<PageContent>.redirects(keep: Boolean = true): Flow<PageContent> =
    filter { it.isRedirect == keep }
