...
public object Diffs {
     ...
     *
     * Lines are split without their terminators and the diff supplies its own. Keeping the
     * terminators makes a page whose last line has no trailing newline render its `-` and the
     * following `+` on one physical line, which reads as content loss when the only change is an
    ...
    public fun unified(
        ...
        return UnifiedDiffUtils
            .generateUnifiedDiff("old/$title", "new/$title", original, patch, context)
            .joinToString("\n")
    }
}
