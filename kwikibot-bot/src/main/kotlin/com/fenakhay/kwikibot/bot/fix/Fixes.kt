 ...
 * @param scope which text the replacements may touch. The default is prose, so a fix cannot rewrite a
 *   template parameter or the target of a link by accident — the mistake that turns a typo fix into a page
 *   move.
 */
 ...
 *
 * Only fixes that hold for wikitext generally are registered here. Per-language typo lists are left to the
 * bot that maintains them, since whether a string is a typo depends on the wiki's language; [register] adds
 * them.
 */
public object Fixes {
     ...
     *
     * Four or more are left alone: they usually mark an ellipsis of omission with its own convention.
     */
    public val ELLIPSIS: Fix =
        Fix(
            name = "ellipsis",
     ...
     *
     * @throws IllegalArgumentException naming the ones that exist, since a mistyped fix that silently does
     *   nothing looks exactly like a fix that found nothing to do.
     */
