 ...
 *
 * A function of a wiki rather than a ready-made [Flow], so a source can be named on a command line before a
 * session exists to run it against.
 */
 ...
 *
 * The syntax is `kind:argument`: `cat:English lemmas`, `page:volcano`, `transcludes:Template:col`.
 *
 * ```
...
public object PageSourceSpec {
     ...
     *
     * @throws IllegalArgumentException if the kind is not one of [kinds], listing them — a mistyped source
     *   should say what was available, not fail silently on an empty run.
     */
    ...
    public fun parseAll(specs: List<String>): PageSource = PageSource.concat(specs.map { parse(it) })
    ...
    private fun build(kind: String, argument: String): PageSource =
        when (kind) {
            ...
            "recentchanges" ->
                PageSource { wiki ->
                    wiki.logs.recentChanges(limit = argument.toIntOrNull() ?: DEFAULT_RECENT).mapNotNull {
                        it.page
                    }
                }
