...
import com.fenakhay.kwikibot.wikitext.mapNodes
import com.fenakhay.kwikibot.wikitext.Wikitext

/**
 * Removes links to one page, keeping the words that were linked.
 *
 * Run before a page is deleted, so the wiki is not left with red links. `[[volcano]]` becomes
 * `volcano` and `[[volcano|volcanoes]]` becomes `volcanoes`: the rendered text is unchanged, only
 * whether it is a link.
 *
 * Done on the parsed page rather than with a regex, which matters more here than almost
 * anywhere: a regex for `[[volcano]]` also matches inside `[[File:volcano.jpg|thumb|A
 * [[volcano]]]]`, and unlinking there rewrites a caption from the outside in.
 *
 * ```
...
public class Unlinker(
     ...
     *
     * `$wgCapitalLinks`, and true nearly everywhere except Wiktionary. It decides whether
     * `[[Volcano]]` and `[[volcano]]` are the same link, and getting it wrong on a Wiktionary
     * unlinks the wrong entry.
     */
     ...
     *
     * Every link, at any depth: one inside a file caption or a template parameter is still a
     * link, and leaving it behind would delete the page while the red link stayed.
     */
    public fun unlink(code: Markup, target: Title.Local): Markup =
        code.mapNodes { node -> unlinkIn(node, target) }
    ...
    private fun unlinkIn(node: Node, target: Title.Local): Node = when {
     ...
     *
     * Titles are compared the way MediaWiki compares them: underscores are spaces, surrounding
     * whitespace does not count, and the first letter is case-insensitive where the wiki
     * capitalises it. A section anchor is ignored — `[[volcano#English]]` is still a link to
     * `volcano`.
     */
     ...
     *
     * A piped link shows its text; a plain one shows its target. `[[volcano|]]` — the pipe trick
     * — is written by an editor and expanded by MediaWiki, so the empty text means the target.
     */
