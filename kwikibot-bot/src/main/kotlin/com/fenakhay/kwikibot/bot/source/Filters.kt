 ...
 *
 * `Flow.filter` covers the simple cases. These are the operators with a rule worth implementing once:
 * deduplication, subpage depth, and the two-stream operations.
 */
...
public fun Flow<PageRef>.titleMatching(pattern: Regex): Flow<PageRef> = filter {
    pattern.containsMatchIn(it.title.text)
}

/** Only pages whose title does not match [pattern]. */
public fun Flow<PageRef>.titleNotMatching(pattern: Regex): Flow<PageRef> = filter {
    !pattern.containsMatchIn(it.title.text)
}
 ...
 *
 * Two sources overlap more often than not — a category and a template both name the same page — and editing
 * it twice in one run means the second edit reverts nothing and logs a confusing no-change. The seen set
 * grows with the stream, which is the price of the guarantee.
 */
 ...
 *
 * `0` keeps base pages only, which is how a bot avoids archives and sandboxes without listing them.
 */
public fun Flow<PageRef>.subpageDepthAtMost(maxDepth: Int): Flow<PageRef> = filter {
    it.title.text.count { character -> character == '/' } <= maxDepth
}

/**
 * Only pages that [other] also names.
 *
 * [other] is collected in full first, because an intersection cannot be decided page by page. That makes it
 * the one operator here that is not lazy, so put the smaller stream second.
 */
