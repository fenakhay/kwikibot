...
class FiltersTest {
    ...
    private fun counting(texts: Map<String, String>) = CountingPages(FakePageService(texts, wiki))
    ...
    @Test
    fun `intersecting keeps what both streams name, and excluding keeps what only one does`() = runTest {
        refs("a", "b", "c").intersect(refs("b", "c", "d")).toList().map { it.title.text } shouldBe
            listOf("b", "c")

        refs("a", "b", "c").excluding(refs("b")).toList().map { it.title.text } shouldBe listOf("a", "c")
    }
    ...
    fun `titles are filtered by pattern in both directions`() = runTest {
        ...
        pages.titleNotMatching(Regex("^volcan")).toList().map { it.title.text } shouldBe listOf("mountain")
    }
    ...
    fun `text filters look at content, not titles`() = runTest {
        ...
        contents.textMatching(Regex("\\{\\{col")).toList().map { it.ref.title.text } shouldBe listOf("a")
        contents.textNotMatching(Regex("\\{\\{col")).toList().map { it.ref.title.text } shouldBe listOf("b")
    }
