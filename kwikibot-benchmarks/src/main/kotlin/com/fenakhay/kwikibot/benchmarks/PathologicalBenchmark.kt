 ...
 *
 * These are not representative of anything and are not meant to be. They are the shapes that took
 * a parser which had no memory of its failed attempts from linear to exponential, and one of them
 * is real: `{{#ifeq:{{pagename}}|x|'''}}` down a quotation template, fifty of them on
 * `Template:RQ:la:Garigliano`, which did not finish parsing at all.
 *
 * The numbers here matter less than the fact that there are numbers. If a change reintroduces
 * exponential backtracking these do not get slower, they stop returning, and the benchmark run
 * hangs rather than reporting a regression.
 */
...
public class PathologicalBenchmark {
    ...
    @Benchmark
    public fun unclosedTemplates(): Int = Wikitext.parse("{{a|".repeat(REPEATS)).nodes.size

    /** A link opening that never closes. */
    @Benchmark
    public fun unclosedWikilinks(): Int = Wikitext.parse("[[a|".repeat(REPEATS)).nodes.size

    /** A tag that never closes, which swallows the rest of a page if the parser guesses. */
    @Benchmark
    public fun unclosedTags(): Int = Wikitext.parse("<ref>".repeat(REPEATS)).nodes.size
