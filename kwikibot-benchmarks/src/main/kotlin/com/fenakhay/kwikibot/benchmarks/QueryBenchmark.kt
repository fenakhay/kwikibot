 ...
 *
 * Separate from [ParserBenchmark] because the pages are parsed once in setup: these measure the
 * queries, not the parse. A bot parses a page once and then asks it several things, so this is
 * the part that multiplies.
 */
...
public class QueryBenchmark {
    ...
    @Benchmark
    public fun templates(): Int = parsed.sumOf { it.templates().size }

    /** Templates of one name, which is how a bot finds the one it maintains. */
    @Benchmark
    public fun templatesByName(): Int = parsed.sumOf { it.templates("col").size }

    /** Every wikilink, which is what a link-maintenance bot walks. */
    @Benchmark
    public fun wikilinks(): Int = parsed.sumOf { it.wikilinks().size }

    /** Every heading, the flat form of the section tree. */
    @Benchmark
    public fun headings(): Int = parsed.sumOf { it.headings().size }

    /** Walking every node, which every unindexed question falls back to. */
    @Benchmark
    public fun allNodes(): Int = parsed.sumOf { markup -> markup.allNodes().count() }

    /** The section tree: what an entry-layout bot navigates before it can place anything. */
    @Benchmark
    public fun outline(): Int = parsed.sumOf { it.outline().subsections.size }

    /** Markup stripped to the text a reader sees. */
    @Benchmark
    public fun visibleText(): Int = parsed.sumOf { it.text.length }
}
