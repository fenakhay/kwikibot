 ...
 *
 * Passwords and tokens are held as plain strings because they must be sent as such; keep them
 * out of source control and read them from configuration or the environment.
 */
public sealed interface Credentials {
     ...
     *
     * The wiki expects the account and the bot-password name joined by `@`, which is what
     * [loginName] produces. Bot passwords are preferred over an account's real password because
     * they carry only the rights granted to that specific bot.
     *
    ...
    public data class BotPassword(
        ...
        override val username: String get() = account

        /** The `lgname` the API expects: `Account@botname`. */
        val loginName: String get() = "$account@$botName"
     ...
     *
     * Sent as a bearer header on every request; there is no login round trip and no session
     * cookie to keep alive.
     */
