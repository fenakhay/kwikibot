 ...
 *
 * Two settings here are not optional in practice. Cookies must be stored, or a bot-password
 * session is lost after the login response. And an OAuth 2.0 bearer header has to be attached
 * to every request, since there is no session to carry it.
 *
...
public object WikiHttpClient {
     ...
     *
     * @param credentials decide whether a bearer header is set; a bot password logs in
     *   later, through [LoginManager], and needs the cookie jar this installs.
