...
public data class ApiRequest(
    ...
    val action: String
        get() = params.getValue(ACTION)

    /** Whether this request changes the wiki, and must therefore be paced as a write. */
    val isWrite: Boolean
        get() = kind == RequestKind.WRITE

    /**
     * Whether this request must be POSTed regardless of its length.
     *
     * Writes must be. So must anything carrying a secret: a URL ends up in proxy logs, server logs and
     * browser history, so a password or token has no business in a query string. MediaWiki enforces this too,
     * refusing `action=login` over GET.
     */
     ...
     *
     * Anything whose name ends in `token` goes last: MediaWiki documents that a truncated request must not be
     * able to end with a valid token, and rejects requests where a token is followed by more parameters.
     */
    public fun ordered(): List<Pair<String, String>> =
        params.entries.sortedBy { it.key.lowercase().endsWith(TOKEN_SUFFIX) }.map { it.key to it.value }
