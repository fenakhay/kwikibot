 ...
 *
 * Tokens are stable for the life of a session, so they are fetched once and reused. They do
 * expire — on logout, on session loss, and when a wiki rotates them — which is what
 * [invalidate] and [withFreshToken] are for.
 *
...
public class TokenStore(private val transport: MediaWikiTransport) {
     ...
     *
     * A `badtoken` is not a programming error: a long-running bot outlives its session. The
     * single retry is what turns that into a hiccup instead of a lost edit.
     */
    ...
    private suspend fun fetch(type: String): String {
        val response = transport.call(
            ApiRequest.of("query", "meta" to "tokens", "type" to type),
        )

        val value = response["query"]?.jsonObject
            ?.get("tokens")?.jsonObject
            ?.get("${type}token")?.jsonPrimitive?.content
            ?: throw WikiError.Api(
                code = "notoken",
