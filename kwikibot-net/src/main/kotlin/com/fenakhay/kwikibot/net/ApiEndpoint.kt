 ...
 * @param scriptPath the path MediaWiki is installed under, `/w` on Wikimedia wikis and often
 *   empty on third-party installs.
...
public data class ApiEndpoint(
    ...
    private val origin: String get() = "${if (secure) "https" else "http"}://$server"

    /** The `api.php` URL every request goes to. */
    val apiUrl: String get() = "$origin$scriptPath/api.php"

    /** The `index.php` URL, for permalinks and human-facing links. */
    val indexUrl: String get() = "$origin$scriptPath/index.php"

    /** The article path for a page title, as it appears in a browser. */
    public fun articleUrl(title: String): String =
        "$origin/wiki/${title.replace(' ', '_')}"
