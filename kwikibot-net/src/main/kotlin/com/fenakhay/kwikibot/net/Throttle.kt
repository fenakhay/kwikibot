 ...
 *
 * Reads and writes are paced separately. On top of that sits a penalty: when a server
 * answers `429` or reports replication lag it can ask for a pause, and that pause holds
 * back reads and writes alike until it elapses.
 *
 * Callers are serialized, so this also bounds concurrency to one in-flight acquisition at a
 * time — a pool of coroutines sharing a throttle produces one evenly spaced request stream
 * rather than bursts. Sharing one instance per wiki is the intended use.
 *
 ...
 * @param timeSource must be the same clock the coroutines are scheduled on. In production that
 *   is the default; under `runTest` pass `testScheduler.timeSource`, or a throttle reading the
 *   real clock while `delay` advances virtual time will double-count every pause.
 */
public class Throttle(
     ...
     *
     * Extends an existing penalty rather than shortening it, so overlapping `Retry-After`
     * responses cannot talk the client into going faster.
     */
    ...
    public companion object {
         ...
         *
         * A function rather than a shared constant on purpose: a throttle carries the penalty a
         * server asked for, so a single shared instance would let one caller slow down another.
         */
