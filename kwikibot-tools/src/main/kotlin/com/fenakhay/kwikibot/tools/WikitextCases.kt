 ...
 *
 * Every title is one no wiki has: `Zqx` prefixes a template, a page and a file name throughout.
 * A template that exists is expanded before `action=parse` reports anything, so `{{t|x}}` on
 * en.wiktionary comes back carrying links to `Module:parameters` and half a dozen other pages
 * that are nowhere in the input. A title nobody has created reports exactly what was written.
 *
 * And every family includes what happens when the construct is left open. `{{`, `[[` and `<ref>`
 * with no terminator are where implementations disagree, where MediaWiki's behaviour is
 * surprising — it re-emits the opening as literal text — and where a parser that guesses instead
 * of checking silently eats the rest of a page.
 */
internal object WikitextCases {
     ...
     *
     * Small and single-purpose on purpose: a case that exercises three constructs at once cannot
     * say which one broke.
     */
