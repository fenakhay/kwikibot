 ...
 *
 * Wrapped so an item id cannot be passed where a property id is meant, which is the mistake
 * that makes a statement assert something unintended.
 */
@JvmInline
 ...
 *
 * Sealed, so handling a statement means handling every kind of value it might hold — and so a
 * value type this library does not model yet arrives as [Unknown] rather than as a crash or a
 * silent empty string.
 */
public sealed interface DataValue {
     ...
     *
     * Precision matters more than the timestamp: `+1952-00-00T00:00:00Z` at precision 9 means
     * "in 1952"; reading it as the first of January invents a day the source did not state.
     */
    public data class Time(
         ...
         *
         * Reading a value more precisely than its precision allows is how a source that
         * said "in 1952" becomes a claim about the first of January.
         */
     ...
     *
     * The amount is kept as written rather than parsed into a number: Wikibase states arbitrary
     * precision, and rounding a stated measurement to fit a Double would lose that.
     */
     ...
     *
     * Kept whole rather than dropped, so a statement carrying one still round-trips and a
     * caller can read it if it knows how.
     */
 ...
 *
 * Wikibase distinguishes three things a statement can say about a property: a value, that there
 * is no value, and that there is a value but it is unknown. Collapsing the last two into `null`
 * loses the difference between "has no children" and "children not recorded".
 */
...
public data class Statement(
    ...
    val property: EntityId get() = mainSnak.property

    /** The value, or `null` when the statement says there is none or that it is unknown. */
    val value: DataValue? get() = (mainSnak as? Snak.Value)?.value
