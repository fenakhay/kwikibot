 ...
 *
 * A name spelled the same everywhere is stored once under `mul` rather than repeated in three
 * hundred languages, which is why an entity can have no `en` label and still have an English one.
 */
 ...
 * A Wikibase entity.
 *
 * Sealed over the kinds this library models, so code that handles entities has to say what it
 * does with a lexeme as well as an item — the two are different enough that treating them alike
 * is usually a bug.
 */
public sealed interface Entity {
     ...
     *
     * The fallback is not a nicety. Wikidata has moved many labels to `mul`, the label that
     * applies to every language at once, and Q42 has no `en` label at all — so reading
     * `labels["en"]` for Douglas Adams returns nothing while the entity plainly has an English
     * label. Anything that asks for one language must look there too.
     */
    public fun label(language: String): String? =
        labels[language]?.value ?: labels[MULTIPLE_LANGUAGES]?.value

    /**
     * The description in [language], or `null`.
     *
     * No `mul` fallback: descriptions distinguish entities *within* a language, so one shared
     * across all of them would defeat the purpose, and Wikibase does not store them that way.
     */
     ...
     *
     * Preferred statements win; deprecated ones are never chosen, since they are kept precisely
     ...
     *
     * Wiktionary's own data lives here, which is why it is modelled rather than deferred: a
     * lexeme has lemmas, a language, a lexical category, forms and senses, none of which an
     * item has.
     */
    public data class Lexeme(
        ...
        override val labels: Map<String, LanguageValue> get() = lemmas
        override val descriptions: Map<String, LanguageValue> get() = emptyMap()
        override val aliases: Map<String, List<LanguageValue>> get() = emptyMap()
     ...
     *
     * The id is the page id of the file description page, not an id of its own, which is why a
     * MediaInfo entity cannot exist without the file it describes.
     */
