 ...
 *
 * Two passes. [Scan] works out what closes, and this walks the text with that answer already in
 * hand — so it never tries a construct to see whether it parses, and never has to undo one. That
 * is the whole design, and it is worth saying why rather than only what.
 *
 * A parser that finds out by trying re-reads everything an abandoned attempt covered, and if
 * abandoned attempts can contain each other the cost doubles per level. `{{#ifeq:{{pagename}}|x|
 * '''}}` down a quotation template is ordinary Wiktionary markup, and fifty of them on one page
 * did not finish parsing. Knowing the answer first is not an optimisation of that; it removes the
 * question.
 *
 * The tokenizer decides what each piece of syntax *is*. [Builder] assembles the tree afterwards,
 * which keeps this a plain left-to-right walk.
 */
internal class Tokenizer {
     ...
     *
     * Buffered rather than emitted so that a run of ordinary characters becomes one [Token.Text]
     ...
     *
     * [Scan] settles whether a construct *closes*, which is the question that used to cost the
     * parser its running time. It does not settle whether the inside is well formed, and two
     * things can still turn out not to be what their opening claimed: a tag whose attributes run
     * into something that is not a `>`, and a bracket pair around something that is not a URL.
     *
     * Backing out of those is safe in a way the old parser's backtracking was not. Neither can
     * contain another of its own kind in the part being retried, so what is re-read is bounded by
     * one construct rather than by the nesting depth of the page.
     *
     * The pending text is kept whole rather than as a length. Emitting a token flushes the buffer
     * into one, so a length taken before that points into the *next* run of text, and restoring
     * it would keep a few characters of whatever came after.
     */
     ...
     *
     * Each scope carries its own terminators, so the content loop stays a dispatch on markup
     * rather than a second switch on scope.
     */
    ...
    private fun ends(scope: Scope, char: Char): Boolean = when (scope) {
        Scope.TEMPLATE_NAME, Scope.TEMPLATE_VALUE, Scope.ARGUMENT_NAME -> char == '|'
     ...
     *
     * [limit] is where the enclosing construct's closing markup begins, and it is known before
     * this is called — which is what lets nested constructs be parsed once each.
     */
     ...
     *
     * Almost every character of almost every page is prose, and prose used to cost a map lookup
     * that boxed the character, a call, and an append of one character to a builder. Here it
     * costs one array read, and the run is copied in a block when it ends.
     *
     * The table is deliberately generous. It says "this character might begin something", not
     * "this character does", so the dispatch above still decides — a `;` that is not at the start
     * of a line, or an `h` that is not the start of `https://`, comes back here having emitted
     * itself and cost only the detour.
     */
     ...
     *
     * Nothing above ASCII is markup, so non-Latin text answers no on a bounds check and runs
     * through at memory speed. A scheme letter answers yes only at the start of a word, since the
     * `s` inside one cannot begin `sftp://`.
     */
     ...
     *
     * A run of braces is several openings, not one, and only some of them may close: in
     * `{{{{x}}}}` the outer brace on each side is text and the three inside are an argument. So
     * each brace is looked at on its own and the run sorts itself out.
     */
     ...
     *
     * A parameter is positional until an `=` turns up in its key, which is why the key is read
     * first and the equals sign decides whether a value follows.
     */
     ...
     * Parses `[url text]`.
     *
     * The brackets close — [Scan] said so — but a bracket pair around something that is not a URL
     * is not a link, and `[not a url]` has to come back as the text it is.
     */
     ...
     *
     * Reached from every letter of every page, so the cheap checks come first: a character that
     * cannot start a scheme costs one array lookup.
     */
     ...
     *
     * Matched against the text in place rather than against a substring: this is consulted at
     * the start of every word of every page, and copying the remainder each time would make
     * tokenizing quadratic in the length of the page.
     *
     * Only the schemes beginning with this character are tried. That sounds like a detail and is
     * not: the schemes start with s, t, i, h, m, w, n, f, b, g and u, which is most of the
     * consonants English words start with, so testing all twenty-one against every such word was
     * the single most expensive thing the tokenizer did.
     */
    private fun schemeAt(position: Int): String? {
        ...
        val candidates =
            if (char.code < SCHEMES_BY_FIRST.size) SCHEMES_BY_FIRST[char.code] else null
     ...
     *
     * Trailing punctuation is left behind: a sentence ending "see https://example.org." links to
     * the site, not to the site plus a full stop.
     */
     ...
     *
     * Tags whose contents MediaWiki does not parse — `nowiki`, `pre` and friends — have their
     * bodies taken verbatim, which is the whole point of writing `<nowiki>{{x}}</nowiki>`.
     */
     ...
     *
     * The padding is not cosmetic detail to be normalised away: a bot that rewrites one attribute
     * must leave the rest of the tag exactly as the last editor typed it.
     */
    ...
    private fun attributeValue() {
        ...
        val value = if (quote == '"' || quote == APOSTROPHE) {
     ...
     *
     * The partner is looked for once, with a plain search, rather than by parsing ahead and
     * giving up. That is the difference between this being linear and it being the reason a real
     * page never finished.
     */
    private fun style(limit: Int) {
        ...
        if (closing < 0 || closing + markup.length > limit) {
            emitText(markup)
            head += markup.length
            return
        }
     ...
     *
     * Whether the line is a heading is decided by looking at the line, before anything in it is
     * parsed — so a line that turns out not to be one is not parsed twice.
     */
     ...
     * Parses `&amp;`, `&#65;` or `&#x41;`.
     *
     * An `&` that does not begin a well-formed entity is ordinary text — `a & b` must survive
     * untouched — so an unrecognised name is emitted as what it is.
     */
    private fun entity() {
        val start = head
        var probe = head + 1
    ...
    private fun isValidEntity(body: String, numeric: Boolean, hexadecimal: Boolean): Boolean = when {
        body.isEmpty() -> false
    ...
    private companion object {
        ...
        val URL_SCHEMES = listOf(
            "https://", "http://", "ftps://", "ftp://", "sftp://", "irc://", "ircs://",
            "gopher://", "telnet://", "nntp://", "worldwind://", "mailto:", "news:", "svn://",
            "git://", "mms://", "bitcoin:", "magnet:", "urn:", "geo:", "//",
        )

        /**
         * The schemes each character can begin, in both cases, or `null` for the characters that
         * begin none.
         *
         * Consulted at the start of every word of every page, which is why it is a lookup into a
         * handful of candidates rather than a search through all of them.
         */
        val SCHEMES_BY_FIRST: Array<List<String>?> = arrayOfNulls<List<String>>(128).also { table ->
         ...
         *
         * Anything not in here and not in [SCHEME_START] is prose, and is copied without being
         * looked at again.
         */
        val MARKUP = BooleanArray(128).apply {
            // Openings.
         ...
         *
         * Kept apart from [MARKUP] because these are letters, and letters are what pages are made
         * of. Between them the schemes begin with s, t, i, h, m, w, n, f, b, g and u, so breaking
         * at every one of those rather than every word-initial one meant breaking on most of the
         * consonants in the language.
         */
        val SCHEME_START = BooleanArray(128).apply {
