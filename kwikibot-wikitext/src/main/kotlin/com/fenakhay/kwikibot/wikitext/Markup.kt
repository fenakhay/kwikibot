 ...
 *
 * Immutable. Every edit returns a new `Markup`, and every node that was not touched keeps the
 * exact text it was parsed from — so a bot that changes one template parameter produces a diff
 * of one template parameter, not of the whole page.
 *
 * ```
...
public class Markup(nodes: List<Node>) {
     ...
     *
     * Templates and comments contribute nothing, since neither is text a reader sees; a wikilink
     * contributes its display text. Use this for comparing content, never for writing back.
     */
    public val text: String
        get() = nodes.joinToString("") { node ->
            when (node) {
                ...
                is Template, is Argument, is Comment -> ""
            }
        }
     ...
     *
     * @param name when given, only templates with this name, compared ignoring the case of the
     *   first letter and treating spaces and underscores alike — the way MediaWiki compares them.
     */
    public fun templates(name: String? = null): List<Template> =
        allNodes().filterIsInstance<Template>()
     ...
     *
     * Nested occurrences are replaced too, so a template inside a link can be edited without the
     * caller having to find and rebuild the link.
     */
     ...
     *
     * The common shape of a bot edit: find the templates that matter, change them, leave
...
public object Wikitext {
     ...
     *
     * Total: no input is rejected. Markup that does not parse — an unclosed template, a stray
     * bracket — stays literal text, which is what MediaWiki renders.
     */
...
internal fun Node.children(): List<Markup> = when (this) {
    ...
    is TextNode, is Comment, is HtmlEntity -> emptyList()
}

/**
 * This node with [target] replaced by [replacement] wherever it appears inside it.
 *
 * Returns the node unchanged when it contains no occurrence, so untouched subtrees keep their
 * identity and their exact text.
 */
private fun Node.rewrite(target: Node, replacement: Node?): Node {
    ...
    return when (this) {
        ...
        is TextNode, is Comment, is HtmlEntity -> this
    }
}
 ...
 *
 * MediaWiki ignores the case of the first letter and treats underscores as spaces, so
 * `{{col}}`, `{{Col}}` and `{{c ol}}` are not all the same but the first two are.
 */
