 ...
 *
 * The shape most wiki work has: find `==English==`, then `===Noun===` inside it, then put
 * something in the right place. A section owns the content directly beneath its heading and the
 * subsections nested under it, so moving or replacing one carries its children with it.
 *
 * @param heading the heading that opened this section; `null` only for the lead, which is
...
public data class Section(
    ...
    val level: Int get() = heading?.level ?: 0

    /** The heading text, trimmed, or `null` for the lead. */
    val title: String? get() = heading?.title?.text?.trim()
    ...
    public val content: Markup get() = Markup(nodes)
     ...
     *
     * Headings are compared trimmed, since `== English ==` and `==English==` are the same
     * section to a reader and to MediaWiki.
     *
     * @param title the heading to find, compared trimmed.
     * @param level when given, only a heading at this level matches — the way to tell a
     *   language section from a part-of-speech section that happens to share a name.
     */
    ...
    public fun children(title: String): List<Section> =
        subsections.filter { it.title == title.trim() }
    ...
    public fun withSubsection(subsection: Section): Section =
        copy(subsections = subsections + subsection)

    /** This section with [target] replaced by [replacement] wherever it is nested. */
    public fun replace(target: Section, replacement: Section): Section = when {
 ...
 *
 * The root is the lead — everything before the first heading — with the top-level sections
 * beneath it. A heading deeper than the one before it opens a subsection; one at the same level
 * or shallower closes as many as it needs to.
 */
