 ...
 *
 * What counts as correct order differs by project, so nothing is reordered unless asked and the comparator is
 * a parameter.
 */
 ...
 *
 * Placed at the end, after the existing categories, which is where every project puts them. A page whose
 * categories are somewhere else keeps them there: moving them would be a second change that was not
 * requested.
 *
...
public fun Markup.removeCategory(name: String, prefix: String = CATEGORY): Markup {
    val target =
        nodes.filterIsInstance<WikiLink>().firstOrNull {
            it.isCategory(prefix) && it.categoryName().equals(name, ignoreCase = true)
        } ?: return this
 ...
 *
 * The sort key belongs to the page's place in the category, not to the category, so a page filed under
 * `Smith, John` stays filed under `Smith, John` when it moves.
 */
 ...
 *
 * Alphabetical by code unless a project says otherwise, which is what most now use: the ordering conventions
 * that differed by project were mostly abandoned when interlanguage links moved to Wikidata, and the links
 * that remain are hand-added exceptions.
 *
 * Only the links move. Whatever sat between them — comments, templates, blank lines — stays where it was,
 * because it is usually a note about one of them.
 */
 ...
 *
 * Leaving the newline behind turns a removal into a growing gap at the bottom of the page, one blank line per
 * category ever removed.
 */
