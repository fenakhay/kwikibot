 ...
 *
 * The traversal is here rather than on [Markup] because it is what a tidying pass needs and
 * nothing else does: an edit that knows which node it wants uses `replace`.
 *
 * It does not descend into a tag whose contents MediaWiki does not parse. What is inside
 * `<nowiki>`, `<pre>` or `<syntaxhighlight>` is content: decoding an entity there changes what
 * the page displays, and stripping whitespace there alters preformatted content.
 */
...
private fun Node.mapChildren(transform: (Node) -> Node): Node = when (this) {
    is Template -> copy(
