 ...
 *
 * Tokens are a flat stream, not a tree: the tokenizer decides what each piece of syntax *is*, and the builder
 * assembles the tree afterwards. Splitting the job in two is what makes the tokenizer a plain state machine
...
internal sealed interface Token {
     ...
     *
     * @param brackets whether the link was written as `[url text]` rather than appearing bare in running
     *   text, which decides how MediaWiki renders it.
     */
     ...
     *   for a list item. Absent for a real `<tag>`, which is how the two are told apart when the text is
     *   written back out.
     */
     ...
     *
     * The three paddings are the whitespace around the attribute, kept so the tag can be rebuilt byte for
     * byte: `<ref name = "a">` differs from `<ref name="a">` only in these.
     */
     ...
     * @param implicit whether the tag closed with a bare `>` rather than `/>`, as `<br>` does. Without this
     *   the two forms are indistinguishable, and a bot editing a page would silently rewrite every `<br>` as
     *   `<br/>`.
     */
