 ...
 *
 * This is the half of the parser that makes the other half straightforward. Wikitext is full of openings that
 * turn out not to be openings — `{{`, `[[` and `<ref>` are ordinary text when nothing closes them, which is
 * what MediaWiki renders for `{{Zqx template` and `a <ref>never ends`. A parser that finds that out by trying
 * to parse the construct and giving up has to undo the attempt, and everything it read on the way is then
 * read again by whatever it fell back to. Nest that and the cost doubles per level: a real Wiktionary
 * quotation template with fifty unclosed constructs in it never finished parsing at all.
 *
 * So closure is decided first, in one left-to-right pass with an explicit stack, and the parser that follows
 * never guesses. It asks [closerOf] where a construct ends, is told, and either parses it or emits an opening
 * as the text it is. Nothing is parsed twice, and the exponent is not tamed but absent.
 *
 * This is roughly what MediaWiki's own preprocessor does, which is the other reason to do it: matching by
 * counting brace runs is the behaviour the recorded corpus pins, and it is far easier to match a rule than to
 * match the consequences of one.
 */
internal class Scan private constructor(private val text: String) {

    /**
     * Where each opening construct ends, indexed by the offset it starts at, stored one higher than it is.
     *
     * The offset is kept off by one so that zero can mean "nothing closes here", which is the answer almost
     * everywhere. A sentinel of `-1` would read better and would cost a second pass over the whole array to
     * write it: the JVM has already zeroed this once, and filling it again is one of the few things in the
     * parser that is genuinely proportional to page size without doing any work.
     */
    private val ends = IntArray(text.length)

    /**
     * How many braces open the construct at each offset: three for an argument, two for a template, zero for
     * everything else.
     *
     * A brace run is spent from the right, so the offset alone does not say which of the two opened there —
     * `{{{{{{x}}}}}}` is an argument inside an argument, and both open on the same run. Only braces are
     * ambiguous this way; a `[[` or a `<ref` says what it is.
     */
     ...
     *
     * Only meaningful at an offset where a construct actually opens. Anywhere else the answer is [UNMATCHED],
     * which is also the answer for an opening that never closes, and the two do not need telling apart: both
     * mean "there is no construct here".
     */
    ...
    fun braceWidth(start: Int): Int = if (start in braceWidths.indices) braceWidths[start].toInt() else 0

    /**
     * One unclosed opening, waiting for its partner.
     *
     * [braces] is how many of a brace run are still unspent — five braces can close as three then two — and
     * is zero for everything else. [name] is a tag's name, and empty otherwise.
     */
     ...
     *
     * Split out because the parallel path needs the same decisions taken in the same order over a list of
     * offsets rather than over the text.
     */
     ...
     *
     * `<!-- {{ -->` contains no template, and a page whose braces balance only because a comment supplied one
     * of them would be parsed quite differently from the way it renders.
     */
     ...
     *
     * The whole purpose of `<nowiki>{{x}}</nowiki>` is that the braces inside it are not a template, so they
     * must not be matched as one.
     */
     ...
     *
     * A run is not one opening but several, and the rightmost braces open the innermost construct: `{{{1}}}`
     * is an argument, not a template whose name is `{1`. So the run is pushed whole and spent from the right
     * as closers arrive.
     */
     ...
     *
     * Three close an argument and two a template, preferring the argument, which is how `{{{1|x}}}` is an
     * argument rather than a template containing a stray brace. Both runs are spent from the right, so five
     * braces closed by five give an argument inside a template - and `{{{{{|safesubst:}}}x}}` is exactly that
     * on real pages.
     */
    private fun closeBraceRun(open: ArrayDeque<Open>, start: Int): Int {
        ...
        while (available >= TEMPLATE_BRACES) {
            ...
            val spend =
                if (available >= ARGUMENT_BRACES && frame.braces >= ARGUMENT_BRACES) {
                    ARGUMENT_BRACES
                } else {
                    TEMPLATE_BRACES
                }
     ...
     *
     * Only the innermost: in `[[a{{b]]` the `]]` does not reach past the template that opened after it, and
     * the link is left unmatched, which is what MediaWiki shows.
     */
     ...
     *
     * Only well-formed names are considered, so a `<` in running text costs a character-class check and
     * nothing more.
     */
     ...
     *
     * Quoting is respected, so the `>` in `<span title="a > b">` does not end the tag early. A second `<`
     * does end the search: whatever the first one started, it was not a tag.
     */
    ...
    internal companion object {
        ...
        private val RAW_CONTENT_TAGS = setOf("nowiki", "pre", "syntaxhighlight", "source", "math", "score")
