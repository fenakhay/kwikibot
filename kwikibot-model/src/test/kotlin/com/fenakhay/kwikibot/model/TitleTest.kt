...
class TitleTest {
    ...
    fun `namespace prefixes resolve by canonical name and alias`() {
        Title.parse("Category:English lemmas")
            .shouldBeInstanceOf<Title.Local>().namespace shouldBe Namespace.CATEGORY
        Title.parse("Image:Foo.png")
            .shouldBeInstanceOf<Title.Local>().namespace shouldBe Namespace.FILE
        Title.parse("template talk:col")
            .shouldBeInstanceOf<Title.Local>().namespace shouldBe Namespace.TEMPLATE_TALK
    }
    ...
    fun `first letter casing follows the namespace rule`() {
        ...
        val caseSensitive = NamespaceMap(
            listOf(NamespaceInfo(Namespace.MAIN, "", "", case = TitleCase.CASE_SENSITIVE)),
        )
        Title.parse("volcano", namespaces = caseSensitive)
            .shouldBeInstanceOf<Title.Local>().text shouldBe "volcano"
    }
    ...
    fun `empty and whitespace-only titles are invalid`() {
        ...
        Title.parse("   ").shouldBeInstanceOf<Title.Invalid>().reason shouldBe Title.Invalid.Reason.EMPTY
        Title.parse("#section").shouldBeInstanceOf<Title.Invalid>().reason shouldBe
    ...
    fun `default namespace applies only when no prefix is given`() {
        Title.parse("col", defaultNamespace = Namespace.TEMPLATE)
            .shouldBeInstanceOf<Title.Local>().namespace shouldBe Namespace.TEMPLATE
