 ...
 *
 * A file page has a history like any other page, and each entry here is one upload rather than
 * one edit of the description. The newest is first, which is the order the API returns.
 */
 ...
 *
 * Warnings are their own case rather than an error, because most of them are advisory — a
 * duplicate name, a file already on Commons, a badly formatted name — and the caller has to
 * decide. The file is already on the server when they arrive: publishing it is one more call
 * with the [Warned.fileKey], not another upload.
 */
