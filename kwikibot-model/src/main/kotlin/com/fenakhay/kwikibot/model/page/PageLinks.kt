 ...
 *
 * The title is that wiki's, not this one's. On a Wikipedia they usually differ, since `Volcano` is `Volcan`
 * in French; on a Wiktionary they usually match, since every edition documents the same spelling. A bot
 * working across languages has to read the title rather than assume either.
 */
 ...
 *
 * Distinct from a [LanguageLink]: an interwiki link points sideways at a different project, `w:Etsy` rather
 * than at the same subject in another language.
 */
 ...
 *
 * MediaWiki counts these as it goes rather than on demand, so they are cheap to ask for and occasionally a
 * little behind. Close enough to decide whether a category needs splitting; not a substitute for enumerating
 * it when the exact set matters.
 */
