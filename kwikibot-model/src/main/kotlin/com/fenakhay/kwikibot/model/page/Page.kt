 ...
 *
 * Carried on every page reference so a value taken from one wiki cannot be handed to another — the mistake
 * that turns a Commons lookup into an edit on the local wiki.
 */
@JvmInline
 ...
 *
 * A pure value: constructing one performs no I/O and says nothing about whether the page exists. Content is
 * fetched explicitly, which is what keeps request counts visible.
 */
public data class PageRef(
     ...
     *
     * Absent for a reference built from a title alone, and stable across moves where a title is not, so it is
     * the safer thing to record in a log.
     */
 ...
 *
 * Editing takes [revisionId] back as the base revision, which is what lets the wiki detect an edit conflict
...
public data class PageContent(
     ...
     *
     * Pass it back as an edit's base revision: it is what lets the wiki refuse an edit computed from text
