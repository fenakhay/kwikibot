 ...
 *
 * Wikipedias have a page per calendar day, named in the wiki's own language: `January 1` on en, `1er janvier`
 * on fr, `1. Januar` on de, `1 de enero` on es. A bot that links or categorises them has to build and read
 * those titles.
 *
 * Covers the languages this library holds month names for; [register] adds another, which takes a month list
 * and a format string. Exhaustive per-language date handling — Roman numerals, local digit systems, era
 * suffixes, non-Gregorian calendars — is out of scope: it served cross-wiki date linking, which Wikidata
 * sitelinks now do.
 */
public object DatePages {
     ...
     *
     * @throws IllegalArgumentException if the month or day is not a real one — a bot that builds `February
     *   31` links to a page that cannot exist.
     */
     ...
     *
     * Matching is case-insensitive on the month, since a title arrives capitalised on wikis where MediaWiki
     * capitalises the first letter and not on the ones where it does not.
     */
    public fun parseDayTitle(title: String, language: String = "en"): Pair<Int, Int>? {
        val format = formats[language] ?: return null

        val monthIndex =
            format.months.indexOfFirst { month ->
     ...
     */
    public fun daysIn(month: Int): Int =
        when (month) {
            FEBRUARY -> LEAP_FEBRUARY
            APRIL,
            JUNE,
            SEPTEMBER,
            NOVEMBER -> SHORT_MONTH
            else -> LONG_MONTH
        }
