 ...
 *
 * Parsing is total: every input produces one of the three cases rather than throwing, so an
 * off-wiki target cannot be mistaken for a local page. A bot that only ever edits its own wiki
 * handles [Local] and reports the rest.
 */
public sealed interface Title {
    ...
    public data class Local(
         ...
         *
         * Custom namespaces have no canonical name without the site info of the wiki, so they
         * render as `ns118:` rather than guessing. Use [NamespaceMap.format] for display.
         */
     ...
     *
     * A distinct case rather than a local title, so an interwiki target cannot reach a write
     * path that expects a page on this wiki.
     */
    ...
    /** A target MediaWiki would reject. */
        ...
        override val fragment: String? get() = null
        ...
        public enum class Reason {
            ...
            /**
             * Contains a URL percent-escape such as `%C3`. MediaWiki forbids these in titles
             * so that a title and its URL encoding cannot name different pages.
             */
            PERCENT_ESCAPE,
             ...
             *
             * `Category:Template:Foo` is fine by contrast — only the talk namespace rejects a
             * following namespace prefix.
             */
            TALK_OF_NON_MAIN,
        }
    }
    ...
    public companion object {
         ...
         *
         * Applies the normalizations MediaWiki performs — underscores to spaces, whitespace
         * collapsing, HTML entity decoding, a single leading colon stripped, and the
         * first-letter casing rule of the resolved namespace — then classifies the result.
         * Percent-escapes are rejected rather than decoded, as MediaWiki does.
         *
         * @param raw the title as it was written, before any normalisation.
         * @param namespaces the namespaces of the wiki; defaults to the canonical English
         *   set.
         * @param interwiki the interwiki prefixes of the wiki; defaults to none, which
         *   makes every prefix either a namespace or plain title text.
        ...
        private fun localTitle(
            ...
            validate(text)?.let { return Invalid(raw, it) }

            val case = namespaces[prefixes.namespace]?.case ?: TitleCase.FIRST_LETTER
            val cased = if (case == TitleCase.FIRST_LETTER) {
         ...
         *
         * On en.wiktionary `Wiktionary` is both the project namespace and an interwiki prefix,
         * and MediaWiki reads `Wiktionary:Todo` as the local project page. Once a namespace is
         * found no further prefix is interpreted, so `Wiktionary:w:Etsy` stays a local page
         * whose name happens to contain a colon.
         */
        ...
        private fun validate(text: String): Invalid.Reason? = when {
        ...
        private fun normalizeWhitespace(text: String): String {
            val spaced = buildString(text.length) {
