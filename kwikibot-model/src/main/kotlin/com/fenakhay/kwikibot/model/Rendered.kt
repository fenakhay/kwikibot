 ...
 *
 * Not the same view as the wikitext parser's own section tree: this is what the wiki produced, so
 * a heading a template generated appears here and not there. Only this view carries [index], and
 * only [index] can be given to an edit, so this is the step before changing one section of a page.
 */
 ...
 *
 * [exists] is the part worth having: `prop=links` on a query does not report it, so finding the
 * red links on a page otherwise costs a second request.
 */
 ...
 *
 * Only what was asked for is filled in; the rest is empty. Every list is what the wiki resolved
 * *after* templates expanded, which is why asking about unsaved wikitext is useful: it answers
 * what an edit would produce before the edit is made.
 */
