 ...
 *
 * A snapshot, like everything else in this module: rights change, blocks expire, and a value read an hour ago
...
public data class UserInfo(
     ...
     *
     * A wiki with temporary accounts enabled auto-creates one for an editor who is not logged in, in place of
     * recording an IP address. It has an id, an edit count and a talk page, so it is neither anonymous nor a
     * person's account: it belongs to a browser session and its holder will most likely never see a message
     * left on it.
     */
     ...
     *
     * False for an IP address and for a temporary account alike, which is the distinction a bot needs before
     * treating an edit history as one person's.
     */
    val isRegistered: Boolean
        get() = !isAnonymous && !isTemporary && !isMissing
}

/**
 * Whether a wiki auto-creates temporary accounts, and how their names are shaped.
 *
 * Reported by `siprop=autocreatetempuser`. The patterns are the wiki's own — `~2$1` on Wikimedia wikis, where
 * `$1` stands for the rest of the name — so a bare username from a recent change or a signature can be
 * classified without hardcoding a shape that differs per wiki.
 */
 ...
 *
 * Distinct from [BlockInfo], which describes the block found *on* a known user and therefore needs no target.
 * An entry in the list has to name who or what is blocked, and that is not always an account: a block can
 * cover a single address or a whole range.
 */
