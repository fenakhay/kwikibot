 ...
 *
 * A wiki refuses edits routinely and for reasons a bot is expected to handle: another editor changed the page
 * first, an abuse filter disagreed, the page is protected. Those are values rather than exceptions, so
 * handling them is a `when` the compiler checks. Genuine faults (no network, bad credentials, a malformed
 * request) still throw [WikiError].
 */
public sealed interface EditOutcome {
     ...
     * @param currentRevision the revision the page is on now, when the wiki reported it; the edit can be
     *   recomputed from that and retried.
     */
     ...
     *
     * MediaWiki reports these separately from a plain conflict because the recovery differs — the page may no
     * longer be there to edit at all.
     */
     ...
     *
     * Carries the API error code, so a caller can still branch on it, and so an unmodelled refusal is visible
     * in logs rather than disguised as one of the cases above.
     */
