 ...
 *
 * Exceptions are for the exceptional. Outcomes a bot is expected to branch on — an edit
 * conflict, a page the abuse filter refused — are values, not throws; see [EditOutcome].
 *
 * The hierarchy is sealed, so a `when` over it is exhaustive and a new failure mode surfaces as
 * a compile error at the places that classify errors.
 */
public sealed class WikiError(
     ...
     *
     * Transport hiccups, replication lag and rate limits are transient; a missing page or a
     * revoked permission is not.
     */
    public open val isTransient: Boolean get() = false

    /** The request never produced a usable response. */
    public sealed class Transport(message: String, cause: Throwable? = null) :
        WikiError(message, cause) {
    ...
    public sealed class Auth(message: String) : WikiError(message) {
         ...
         *
         * Distinct from [MissingRight] because the wiki said only that we may not do this,
         * which is all a caller can report.
         */
         ...
         *
         * Transient: tokens expire, and the transport refetches and retries once before this
     ...
     *
     * Transient by nature: read-only windows are how MediaWiki survives maintenance, and the
     * same request usually succeeds once the window closes.
     */
