 ...
 *
 * Shared fields are on the entry; the type-specific ones are a sealed [LogDetails] beside it.
 * Composition rather than a subclass per log type, so reading a log needs no downcast and an
 * unmodelled type still arrives with its fields intact.
 */
public data class LogEvent(
     ...
     *
     * Log entries can be suppressed field by field, so an absent page is a fact about the entry
    ...
    val isSuppressed: Boolean get() = page == null || user == null || comment == null
}

/**
 * The part of a log entry that depends on what kind of entry it is.
 *
 * Sealed, so handling a log means saying what happens for each kind; [Unknown] keeps the raw
 * fields of a log type this library does not model, rather than dropping them.
 */
public sealed interface LogDetails {
    ...
    public data class Block(
         ...
         *
         * Also `null` on an unblock, which has no expiry to report — read the entry's action
         * before reading this.
         */
         ...
         *
         * True for an unblock as well, which carries no expiry either. Check the entry's action
     ...
     *
     * The log records both a rendered description meant for a human and the restrictions
     * themselves; only the second is worth acting on, which is why both are kept.
     */
 ...
 *
 * Recent changes mixes edits, page creations and log entries in one stream, which is why the
 * revision and the log entry are both nullable: an entry has one or the other, never both.
 */
public data class RecentChange(
    ...
    val isLogEntry: Boolean get() = logEvent != null
}
