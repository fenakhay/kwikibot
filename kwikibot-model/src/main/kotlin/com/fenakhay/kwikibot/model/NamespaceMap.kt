 ...
 * @param localName the wiki's own name for it (`Kategoria`), equal to [canonicalName] on
 *   English wikis.
 ...
 *
 * Prefix matching follows MediaWiki: underscores are equivalent to spaces, surrounding
...
public class NamespaceMap(namespaces: Collection<NamespaceInfo>) {
    ...
    private val byPrefix: Map<String, NamespaceInfo> =
        buildMap {
    ...
    public companion object {
         ...
         *
         * Useful for tests and for parsing before a wiki's own site info has been fetched;
         * real wikis add local names, aliases and custom namespaces on top.
         */
        public val CANONICAL: NamespaceMap = NamespaceMap(
            listOf(
                ...
                NamespaceInfo(
                    Namespace.FILE_TALK, "File talk", "File talk", aliases = listOf("Image talk"),
                ),
 ...
 *
 * A recognised prefix means the target is not a page on this wiki — the distinction that keeps
 * a bot from editing the wrong project.
 *
 * @param prefixes every interwiki prefix the wiki accepts.
 * @param selfPrefixes prefixes pointing back at this same wiki, such as `wikt:` and `en:`
 *   on en.wiktionary. MediaWiki strips those and resolves the rest as an ordinary local
 *   title, so `wikt:volcano` is the page `volcano` rather than a link to another project.
 */
