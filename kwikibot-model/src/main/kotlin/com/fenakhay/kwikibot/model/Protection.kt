 ...
 *
 * MediaWiki spells "no end" several ways (`infinity`, `infinite`, `indefinite`, `never`), which
 * a plain timestamp type has to represent as a sentinel date. Making it a case instead means
 * "does this ever expire" is a `when`, not a comparison against a magic year.
 */
public sealed interface Expiry {
    ...
    public companion object {
         ...
         *
         * @throws IllegalArgumentException if [raw] is neither a MediaWiki timestamp nor one of
         *   its spellings of "never".
         */
...
public data class Protection(
    ...
    public fun isActiveAt(now: Instant): Boolean = when (expiry) {
        Expiry.Never -> true
