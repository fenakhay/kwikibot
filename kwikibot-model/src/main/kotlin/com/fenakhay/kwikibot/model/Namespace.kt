 ...
 *
 * Numbers rather than names are the stable identity: a namespace's local name and aliases vary
 * per wiki, but `0` is main space everywhere. Names are resolved against a wiki's site info.
 */
@JvmInline
public value class Namespace(
     ...
     *
     * Talk namespaces are the odd-numbered ones; the negative namespaces (`Media`, `Special`)
     * are virtual and have no talk space at all.
     */
    public val isTalk: Boolean get() = id >= 0 && id % 2 == 1

    /** Whether this namespace is virtual — it has no pages of its own in the database. */
    public val isVirtual: Boolean get() = id < 0
    ...
    /**
     * The subject (non-talk) namespace associated with this one, or `null` for virtual
     * namespaces. Returns the receiver when it is already a subject namespace.
     */
     ...
     *
     * A wiki may define more, which have no constant here because their numbers are the wiki's
     * own; read those from its site info through `NamespaceMap`.
     */
