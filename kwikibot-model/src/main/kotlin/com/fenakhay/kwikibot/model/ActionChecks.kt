 ...
 *
 * Worth reading before a run over pages a bot does not control. A refused write still costs a
 * request, a log line and a token, and the wiki will happily answer the question in the same
 * query that fetched the page.
 */
public data class ActionChecks(
    /**
     * Each action that was tested, mapped to the error codes refusing it.
     *
     * An empty list means the action is permitted; `protectedpage` and `cascadeprotected` are
     * the codes seen most often on a refusal.
     */
