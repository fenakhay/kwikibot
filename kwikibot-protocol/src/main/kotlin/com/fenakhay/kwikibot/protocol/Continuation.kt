 ...
 *
 * MediaWiki answers a list query with as many results as it feels like and a `continue` object
 * describing where to resume. Following that is the single most repeated piece of plumbing in a
 * bot, so it lives here once, as a cold [Flow]: nothing is requested until the flow is
 * collected, and abandoning the collection stops the paging.
 *
...
public class Continuation(private val transport: MediaWikiTransport) {
     ...
     * @param maxBatches a ceiling on the number of round trips, guarding against a query
     *   that continues forever. `null` means no ceiling.
     */
     ...
     * @param listName the `list=` module name, which is also the key its results appear
     *   under.
     ...
     *
     * A page can arrive in several batches, each carrying different properties, so callers that
     * need whole pages should merge by page id rather than assume one entry per page.
     */
    ...
    private companion object {
         ...
         *
         * Only the modern `continue` object is read. The `query-continue` form belongs to
         * MediaWiki 1.25 and earlier, which no supported wiki still runs.
         */
        fun JsonObject.nextBatchParams(): Map<String, String>? =
            this["continue"]?.jsonObject
         ...
         *
         * `formatversion=2` returns lists, but a few modules still answer with an object, and
         * one shape crashing on the other is a classic source of bot breakage.
         */
        fun kotlinx.serialization.json.JsonElement.asObjectList(): List<JsonObject> = when (this) {
            is kotlinx.serialization.json.JsonArray -> map { it.jsonObject }
