 ...
 *
 * MediaWiki spells these as a pipe-joined list in which a leading `!` means "not": `rcshow` of `bot|!minor`
 * asks for bot edits that are not minor. Three states rather than two matters — not mentioning `minor`
 * returns both minor and non-minor edits, while `!minor` excludes them, and a `Map<String, Boolean>` cannot
 * say the difference between "off" and "unset".
 *
 * ```
...
public class OptionSet private constructor(private val states: Map<String, Boolean>) {
    ...
    public fun on(vararg option: String): OptionSet = OptionSet(states + option.associateWith { true })

    /** Requires [option] to be false. */
    public fun off(vararg option: String): OptionSet = OptionSet(states + option.associateWith { false })
     ...
     *
     * `null` rather than an empty string, because an empty `show=` is not the same as no `show=` on every
     * module.
     */
    public fun toParam(): String? =
        states.entries
    ...
    public companion object {
         ...
         */
        public fun parse(value: String): OptionSet =
            OptionSet(
                value
                    .split('|')
