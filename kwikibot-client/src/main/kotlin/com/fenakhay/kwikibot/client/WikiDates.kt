 ...
 *
 * An internationalisation library would answer a different question. CLDR says what French says;
 * MediaWiki's message files are translated separately and are what the wiki actually writes, and
 * where they differ the wiki is right by definition — it is the thing producing the text being
 * parsed. So the names come from `allmessages` and the zone from siteinfo, and the tables in
 * `kwikibot-model` stay as the offline answer for a dump reader or a test with no session.
 *
 * ```
 ...
 *
 * Both forms are included where a language has them. Polish writes `styczeń` on its own and
 * `stycznia` in a date, Russian `январь` and `января`; a parser that knows only one of the two
 * fails on every signature or half of every date page.
 */
 ...
 *
 * The shape of a signature — which field comes first, whether a comma follows the time — is a
 * MediaWiki date format that the API does not publish, so the patterns still come from the small
 * table in [Signatures]. What the wiki supplies is everything that varies more: the month names
 * and the zone the times are written in.
 *
 * That zone is why this exists. A signature on de.wikipedia reads `21:43, 31. Aug. 2026 (CEST)`
 * and the time in it is Berlin time; reading it as UTC dates the reply two hours early.
 */
 ...
 *
 * Only the month names come from the wiki. How a day page is *named* is a decision that
 * community made — de.wikipedia writes `15. Januar` where CLDR would say `15 Januar`, es
 * `15 de enero` where CLDR says `15 enero`, and fr `1er janvier` for the first of the month — so
 * the pattern stays in the registered format and only the words are replaced.
 */
