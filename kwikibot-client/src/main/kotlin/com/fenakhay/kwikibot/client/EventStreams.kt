 ...
 *
 * Wikimedia's stream carries every wiki at once, which is why [wiki] is on every event: a bot
 * watching one project has to filter, and filtering on the wrong field means acting on another
 * wiki's edits.
 */
public data class WikiEvent(
     ...
     *
     * Kept so a bot that stops can resume from where it left off instead of from now, which is
     * the difference between missing an hour of edits and not.
     */
 ...
 *
 * Server-sent events: one long-lived HTTP response that never finishes, with events separated by
 * blank lines. The framing is handled here rather than by a plugin, because it is a dozen lines
 * and doing it in the open makes the stream testable without a server.
 *
 * ```
 ...
 * ```
 *
 * The flow does not end on its own. Cancel the collecting coroutine to stop it; a dropped
 * connection surfaces as an exception rather than a silent end, so a caller cannot mistake a
 * network failure for a quiet wiki.
 */
public class EventStreams(
     ...
     *
     * @param since where to resume from: an offset from a previous [WikiEvent.offset], or a
     *   timestamp. `null` starts from now.
     */
    public fun recentChanges(since: String? = null): Flow<WikiEvent> =
        events("recentchange", since)

    /** Page creations, as they happen. */
    public fun pageCreations(since: String? = null): Flow<WikiEvent> =
        events("page-create", since)
    ...
    public fun events(stream: String, since: String? = null): Flow<WikiEvent> = flow {
        client.prepareGet("$baseUrl/v2/stream/$stream") {
     ...
     *
     * The format is one field per line, a blank line ending an event. Everything else — comment
     * lines starting with a colon, which the server sends as a keep-alive, and unrecognised
     * fields — is ignored rather than treated as an error.
     *
     * An event with no blank line after it is not emitted. That is the specified behaviour and
     * the safe one: a connection cut mid-event would otherwise deliver half a payload as though
     * it were whole.
     */
     ...
     *
     * A payload this library cannot read is skipped rather than thrown: the stream is public and
     * shared, and one malformed event is no reason to end a bot run that has been watching for
     * hours.
     */
