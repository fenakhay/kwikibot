 ...
 *
 * The numbers are the ones the ProofreadPage extension stores, and the order is the order of progress — which
 * is why this is comparable: a bot that only touches pages below [PROOFREAD] says so as a comparison rather
 ...
 *
 * A Wikisource page is one scanned page of a book, and its wikitext has a fixed shape: a `<noinclude>` header
 * carrying the quality marker and running heads, the body, and a `<noinclude>` footer. Only the body is
 * transcluded into the book.
 *
 * The split matters because the parts have different owners. A bot fixing OCR touches the body; the quality
 * marker records who proofread the page and when, and rewriting it would claim their work. [serialize] puts
 * back exactly what it was given for the parts that were not changed.
 */
public data class ProofreadText(
    ...
    public companion object {
         ...
         *
         * A page not in this shape — hand-written, or from a wiki without the extension — comes back as all
         * body. That reading loses nothing: rewriting the body then rewrites the whole page rather than
        ...
        public fun parse(wikitext: String): ProofreadText {
            ...
            val closing =
                CLOSING.find(wikitext, startIndex = opening.range.last)
                    ?: run {
            ...
            return ProofreadText(
                ...
                quality = quality?.groupValues?.get(1)?.toIntOrNull()?.let { ProofreadQuality.of(it) },
        ...
        private val QUALITY = Regex("""<pagequality level="(\d)" user="([^"]*)"\s*/>""")
    }
}
 ...
 *
 * Needs the ProofreadPage extension, which only Wikisource and its sisters have; every method refuses without
