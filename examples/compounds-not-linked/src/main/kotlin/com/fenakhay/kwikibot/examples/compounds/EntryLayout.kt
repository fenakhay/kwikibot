 ...
 *
 * Navigating the section tree rather than working in character offsets: the parser
 * round-trips byte for byte, so serialization puts back the parts an edit did not touch
 * without anything here having to preserve them.
 */
public object EntryLayout {
     ...
     *
     * Anything not listed is not a part of speech, which is what keeps `References`,
     * `Further reading` and `Anagrams` from being treated as one.
     */
    public val POS_HEADINGS: Set<String> = setOf(
        "abbreviation", "acronym", "adjective", "adnominal", "adverb", "affix", "article",
        "circumfix", "classifier", "conjunction", "contraction", "counter", "determiner",
        "ideophone", "idiom", "infix", "initialism", "interfix", "interjection", "letter",
        "noun", "numeral", "number", "ordinal number", "participle", "particle", "phrase",
        "postposition", "prefix", "preposition", "prepositional phrase", "pronoun",
        "proper noun", "proverb", "punctuation mark", "root", "suffix", "syllable", "symbol",
        "verb",
    )
     ...
     */
    private val POS_SUBSECTIONS = listOf(
        "usage notes", "reconstruction notes", "inflection", "declension", "conjugation",
        "alternative forms", "alternative reconstructions", "synonyms", "antonyms",
        "hypernyms", "hyponyms", "meronyms", "holonyms", "troponyms", "coordinate terms",
        "derived terms", "related terms", "collocations", "descendants", "translations",
    )

    /**
     * Headings that close out a part-of-speech section.
     *
     * WT:EL puts References, Further reading and Anagrams at the language level and does not
     * mention Statistics at all — but all four occur as POS children in practice, `{{rank}}`
     * alone putting a Statistics section in some 43,000 entries. Every one of them belongs
     * after Derived terms, and listing them is what stops [rank] calling them unknown: an
     * unknown heading is not treated as a successor, so a new section would be appended below
     * them instead of above.
     */
    private val TRAILING_SUBSECTIONS = listOf(
        "statistics", "see also", "references", "further reading", "anagrams",
    )
    ...
    private val CATEGORY_TEMPLATES = setOf(
        "cln", "c", "catlangname", "catlangcode", "topics", "top", "defaultsort",
    )
    ...
    /**
     * Whether the entry uses `===Etymology 1===` numbering, which pushes parts of speech down
     * a level.
     */
    ...
    public fun posSections(language: Section): List<Section> {
        ...
        return language.all()
            .filter { it.level == level && it.title?.lowercase() in POS_HEADINGS }
            .toList()
    }
     ...
     *
     * Unknown headings rank before everything, so a new section is never inserted above a
     * heading the bot does not understand — it appends after instead.
     */
     ...
     *
     * Categories, topic templates and a `----` rule belong at the very bottom of a language
     * section, so a subsection appended at the end has to land above them rather than below.
     */
     ...
     *
     * Walks back over whole lines, terminators included, so the newline that ends the last real
     * line stays with it rather than being mistaken for a blank line of its own.
     */
     ...
     *
     * The first child that should follow it decides: the new section goes directly before that
     * one. When nothing should follow, it goes last.
     */
    ...
    private fun Node.isFurniture(): Boolean = when (this) {
        // Whitespace between furniture is furniture too, but real text is not.
        is TextNode -> text.isBlank() || text.trim().all { it == '-' } && text.trim().length >= RULE_LENGTH
